...
const slugify = (value: unknown) =>
  String(value || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().trim().replace(/[^a-z0-9]+/g, '-')
    .replace(/^-|-$/g, '');
...
Deno.serve(async (request) => {
  ...
  try {
    if (action === 'list') {
      const [{ data: photos, error }, { data: categories, error: categoryError }, { data: settings }] = await Promise
        .all([
          supabase.from('portfolio_photos').select('*, portfolio_categories(id, slug, label, sort_order)').order(
            'sort_order',
          ).order('created_at'),
      ...
      const hydrated = await Promise.all((photos || []).map(async (photo) => {
        ...
        if (!thumbnailPath) {
          return { ...photo, thumbnail_url: photo.legacy_public_url, web_url: photo.legacy_public_url };
        }
      ...
      const counts = (categories || []).map((category) => ({
        ...
        selected: (photos || []).filter((photo) =>
          photo.category_id === category.id && photo.is_published && photo.show_in_category
        ).length,
      }));
      return json({
        photos: hydrated,
        ...
        counts,
        allSelected: (photos || []).filter((photo) =>
          photo.is_published && photo.show_in_all
        ).length,
    ...
    if (action === 'save') {
      ...
      const { data: existing, error: existingError } = await supabase.from('portfolio_photos').select(
        'category_id,show_in_all,show_in_category,all_sort_order,category_sort_order',
      ...
      if (photo.showInAll && (!existing.show_in_all || existing.all_sort_order == null)) {
        const { data: lastAll } = await supabase.from('portfolio_photos').select('all_sort_order').not(
          'all_sort_order',
          'is',
          null,
        ).order('all_sort_order', { ascending: false }).limit(1).maybeSingle();
        updates.all_sort_order = Number(lastAll?.all_sort_order || 0) + 10;
      }
      if (
        photo.showInCategory &&
        (!existing.show_in_category || existing.category_id !== photo.categoryId ||
          existing.category_sort_order == null)
      ) {
        const { data: lastCategory } = await supabase.from('portfolio_photos').select('category_sort_order').eq(
          'category_id',
          photo.categoryId,
        ).not('category_sort_order', 'is', null).order('category_sort_order', { ascending: false }).limit(1)
          .maybeSingle();
    ...
    if (action === 'reorder') {
      ...
      const orderColumn = mode === 'all'
        ? 'all_sort_order'
        : mode === 'category'
        ? 'category_sort_order'
        : 'sort_order';
      for (const [index, item] of items.entries()) {
        const { error } = await supabase.from('portfolio_photos').update({ [orderColumn]: (index + 1) * 10 }).eq(
          'id',
          item.id,
        );
        if (error) throw error;
    ...
    if (action === 'category-create') {
      ...
      const { data: last } = await supabase.from('portfolio_categories').select('sort_order').order('sort_order', {
        ascending: false,
      }).limit(1).maybeSingle();
    ...
    if (action === 'category-reorder') {
      const items = Array.isArray(body.items) ? body.items : [];
      for (const [index, item] of items.entries()) {
        const { error } = await supabase.from('portfolio_categories').update({ sort_order: (index + 1) * 10 }).eq(
          'id',
          item.id,
        );
        if (error) throw error;
    ...
    if (action === 'category-delete') {
      const { count } = await supabase.from('portfolio_photos').select('id', { count: 'exact', head: true }).eq(
        'category_id',
        body.categoryId,
      );
    ...
    if (action === 'create-record') {
      ...
      const { data: last } = await supabase.from('portfolio_photos').select('sort_order').order('sort_order', {
        ascending: false,
      }).limit(1).maybeSingle();
      const [{ data: lastAll }, { data: lastCategory }] = await Promise.all([
        source.showInAll
          ? supabase.from('portfolio_photos').select('all_sort_order').not('all_sort_order', 'is', null).order(
            'all_sort_order',
            { ascending: false },
          ).limit(1).maybeSingle()
          : Promise.resolve({ data: null }),
        source.showInCategory
          ? supabase.from('portfolio_photos').select('category_sort_order').eq('category_id', source.categoryId).not(
            'category_sort_order',
            'is',
            null,
          ).order('category_sort_order', { ascending: false }).limit(1).maybeSingle()
          : Promise.resolve({ data: null }),
      ]);
      const { data, error } = await supabase.from('portfolio_photos').insert({
        id,
        category_id: source.categoryId,
        source_photo_id: source.sourcePhotoId || null,
        source_gallery_id: source.sourceGalleryId || null,
        web_path: `portfolio/${id}/web.webp`,
        thumbnail_path: `portfolio/${id}/thumb.webp`,
        alt_text: String(source.altText || '').slice(0, 280),
        internal_title: String(source.internalTitle || '').trim().slice(0, 120) || null,
        is_featured: Boolean(source.isFeatured),
        is_published: Boolean(source.isPublished),
        show_in_all: Boolean(source.showInAll),
        all_sort_order: source.showInAll ? Number(lastAll?.all_sort_order || 0) + 10 : null,
        show_in_category: Boolean(source.showInCategory),
        category_sort_order: source.showInCategory ? Number(lastCategory?.category_sort_order || 0) + 10 : null,
        sort_order: Number(last?.sort_order || 0) + 10,
        width: Number(source.width) || null,
        height: Number(source.height) || null,
        size_bytes: Number(source.sizeBytes) || null,
    ...
    if (action === 'delete') {
      const { data: photo, error } = await supabase.from('portfolio_photos').select('web_path, thumbnail_path').eq(
        'id',
        body.photoId,
      ).single();
      if (error) throw error;
    ...
    if (action === 'albums') {
      const { data, error } = await supabase.from('albums').select('id, title, event_date, location').order(
        'created_at',
        { ascending: false },
      );
      if (error) throw error;
    ...
    if (action === 'album-photos') {
      const { data, error } = await supabase.from('album_photos').select(
        'id, album_id, filename, caption, thumbnail_path, web_path, storage_path, original_path, width, height, size_bytes',
      ).eq('album_id', body.albumId).order('sort_order');
      if (error) throw error;
