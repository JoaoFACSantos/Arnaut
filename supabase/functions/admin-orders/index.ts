...
import { BUCKET, corsHeaders, errorMessage, json, readJson, sanitizeText } from '../_shared/security.js';
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (body.action === 'invalidate-downloads') {
      ...
      await supabase.from('order_access_tokens').update({ revoked_at: now }).eq('order_id', orderId).is(
        'revoked_at',
        null,
      );
      return json({ ok: true });
    ...
    console.error('admin-orders error', errorMessage(error));
...
async function listOrders(supabase: ReturnType<typeof createServiceClient>, filters: Record<string, unknown>) {
  let query = supabase.from('orders')
    .select(
      'id, public_id, order_number, customer_email, currency, total_cents, status, created_at, paid_at, expires_at, albums(id, title), order_items(id)',
      { count: 'exact' },
    )
  ...
  if (error) throw error;
  return json({
    orders: ((data || []) as unknown as OrderListRow[]).map((order) => ({
      ...order,
...
async function orderDetail(supabase: ReturnType<typeof createServiceClient>, orderId: string) {
  const { data, error } = await supabase.from('orders')
    .select(
      'id, public_id, order_number, customer_email, currency, subtotal_cents, discount_cents, total_cents, status, created_at, paid_at, expires_at, downloads_invalidated_at, email_sent_at, stripe_checkout_session_id, stripe_payment_intent_id, albums(id, title, sales_support_email), order_items(id, photo_id, unit_price_cents, album_photos(filename, thumbnail_path, watermarked_path))',
    )
  ...
  return json({
    order: {
      ...data,
      album: Array.isArray(data.albums) ? data.albums[0] : data.albums,
      items,
      albums: undefined,
      order_items: undefined,
    },
